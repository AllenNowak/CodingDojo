...
function updateInventory(newInv, currInv) {
    ...
    if (newInv.values.length === 0){
        return currInv
    }
    if (currInv.values.length === 0){
        return currInv = newInv
    }
    for (const product of newInv) {
        for (const item of currInv) {
            if (item.name === product.name){
                item.quantity += product.quantity
            } else{
                currInv[product.name] = product.quantity
            }
        }
        // if (currInv.hasOwnProperty(product)){
        //     currInv[product.name].quantity += product.quantity
        // }
        // else {
        //     currInv[product.name].quantity = product.quantity
        // }
